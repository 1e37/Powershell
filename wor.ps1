...
$services = @("wscsvc", "OneSyncSvc", "MessagingService", "PimIndexMaintenanceSvc", "UserDataSvc", "UnistoreSvc", "BcastDVRUserService", "Sgrmbroker")

foreach ($service in $services) {
    $key = Get-ChildItem -Path "HKLM:\SYSTEM\CurrentControlSet\Services" -Recurse | Where-Object { $_.Name -match "\\$service$" }
    if ($key) {
...
$taskbarAlRegistryKey = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarAl"


    # Create the TaskbarAl registry key and set it to 1
    New-Item -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "TaskbarAl" | Out-Null
    Set-ItemProperty -Path $taskbarAlRegistryKey -Name "(Default)" -Value 1
    Write-Host "The TaskbarAl registry key has been created and set to 1."




   # Check if the TaskbarThumbnailClip registry key exists
$taskbarThumbnailClipRegistryKey = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarThumbnailClip"

if (Test-Path -Path $taskbarThumbnailClipRegistryKey) {
    # Set the TaskbarThumbnailClip registry key to 0
    Set-ItemProperty -Path $taskbarThumbnailClipRegistryKey -Name "(Default)" -Value 0
    Write-Host "The TaskbarThumbnailClip registry key has been set to 0."
} else {
    Write-Host "The TaskbarThumbnailClip registry key does not exist."
}
